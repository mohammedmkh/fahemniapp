                ...
                <tbody>
                    @foreach($cities as $key => $country)
                        <tr data-entry-id="{{ $country->id }}">
                            ...
                            <td>
                                @can('country_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.cities.show', $country->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('country_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.cities.edit', $country->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('country_delete')
                                    <form action="{{ route('admin.cities.destroy', $country->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
